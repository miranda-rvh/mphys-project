        ...
        if plot_sim:
            ...
            for i in range(len(experiment_positions)):
                normalisations[i] = charge_density_normalisation_with_reflection_and_smearing(experiment_positions[i], beta, ep, \
                    WAVELENGTH, PULSE_DURATION, REFRACTIVE_INDEX, EFFECTIVE_NUMERICAL_APERTURE)
...
def depth_scan_plot():
    charges, positions, errors = data_file_read(Z_LIMIT, filename = CHARGE_DATA_FILE_NAME, mat_file= SIC_DEPTH_SCAN)

    pickle_file = "pickle_files/envelope.pickle"
    ...
    plt.clf()
    fig, ax = plt.subplots()
    ax.plot(positions*1e6, median_depth_scan_results*1.602e-19*1e15, color = "r", label = r'Theoretical fit, $\beta_2$ ={:.2e} m/W'.format(fit_beta))
    ...
    plt.savefig(PREFIX+"initial_depth_scan_cxl.png")
    #plt.show()
    print("Theoretical Ratio: ", np.max(median_depth_scan_results)/np.max(charges))
    print(fit_beta)

    print('Reduced chi sq:', reduced_chi_squared(charges, errors, median_depth_scan_results, 4))

    return None

def confidence_plot(position, pickle_file = "pickle_files/envelope.pickle", recalculate = False, compute_uniformity=False):
    ...
    ax.fill_between(positions*1e6, min_uniformity_charges*1.602e-19*1e15, max_uniformity_charges*1.602e-19*1e15, color='mediumorchid', alpha=0.5, hatch='\\\\\\', label = 'Range of fluctuations')
    ...
    ax.set_xlim(-60, 110)
    ax.legend(fontsize=20, loc='upper left')
    plt.savefig(PREFIX + "error_envelope_depth_cxl.png")
