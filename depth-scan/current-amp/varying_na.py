...
def direct_charge_density_integrand(z, r, V, beta2, Ep, zr):
    l = WAVELENGTH
    nr = REFRACTIVE_INDEX
    tau = PULSE_DURATION
    h = H
...
def reflected_charge_density_integrand(z, r, V, beta2, Ep, zr):
    l = WAVELENGTH
    nr = REFRACTIVE_INDEX
    tau = PULSE_DURATION
    h = H
...
def interference_charge_density_integrand(z, r, V, beta2, Ep, zr):
    l = WAVELENGTH
    nr = REFRACTIVE_INDEX
    tau = PULSE_DURATION
    h = H
...
def array_charge_density_normalisation_with_reflection_and_smearing(V, beta, ep, zr):
    r_lim = R_LIMIT
    nr = REFRACTIVE_INDEX
    l=WAVELENGTH
    NAv = np.sqrt((nr*l)/(zr*np.pi))
...
def parallel_charge_density(V_values, beta, ep, zr):
    """Parallel computation of charge densities for multiple V values."""

    with mp.Pool(mp.cpu_count()) as pool:
        results = pool.starmap(array_charge_density_normalisation_with_reflection_and_smearing, [(V, beta, ep, zr) for V in V_values])
...
def fit_function(V, beta, ep, zr):
    result = parallel_charge_density(V, beta,ep, zr)

    return np.array(result)

def wrapper_function(zr_test):
    def tempfunc(V, beta,ep, zr=zr_test):
        return fit_function(V, beta,ep, zr_test)
    return tempfunc

def charge_density_normalisation_with_reflection_and_smearing(V, beta, ep, zr):
    r_lim = R_LIMIT
    nr = REFRACTIVE_INDEX
    NAv = np.sqrt(nr*WAVELENGTH/(zr*np.pi))
    REF_INDEX_CORRECTION = 1/np.sqrt((nr**2-NAv**2)/(1-NAv**2))
...
def data_file_read(detector_thickness, recalculate_exp_charges=False, plot = False, plot_sim = False, debug = False, mat_file = SIC_DEPTH_SCAN, filename = CHARGE_DATA_FILE_NAME, beta = BETA_2, NAv = EFFECTIVE_NUMERICAL_APERTURE, ep = PULSE_ENERGY):
    ...
    if plot:  
        ...
        if plot_sim:
            ...
            for i in range(len(experiment_positions)):
                normalisations[i] = charge_density_normalisation_with_reflection_and_smearing(experiment_positions[i], beta, ep, NAv)
...
def curve_fitting(position, save_fig = "plots/depth_scan.png", range_pickle = "pickle_files/numerical_aperture_range_fit.pickle"):
    if __name__ == "__main__":
        ...
        zr_array = np.linspace(zr_min, zr_max, zr_indices)
        NA_array= np.sqrt((REFRACTIVE_INDEX*WAVELENGTH)/(zr_array*np.pi))

        beta_2_values = np.zeros(zr_indices)
        beta_2_errors = np.zeros(zr_indices)
        ...
        for i in range(len(zr_array)):
            popt, pcov = curve_fit(wrapper_function(zr_array[i]), experimental_positions, experiment_n_carriers, bounds = (lower_bounds, upper_bounds), sigma = errors, absolute_sigma = False)
            pbar.update(i+10)
        ...
        pickle.dump([zr_array, NA_array, beta_2_values, beta_2_errors], picklefile)

        return 0
    
def confidence_plot(position, max_zr, min_zr, range_pickle = "pickle_files/numerical_aperture_range_fit.pickle", bound_pickle = "pickle_files/full_data_na_pickle.pickle",recalculate = True):
    beta_data = np.genfromtxt(
            "beta_vals.txt", 
            delimiter=",", 
            dtype=[('coordinate', 'U10'), ('beta', 'f8'), ('error', 'f8')], 
    ...
    zr_array, NA_array, beta_2_values, beta_2_errors = pickle.load(range_pickle_file)
    ...
    if recalculate:
        ...
        upper_popt, upper_pcov = curve_fit(wrapper_function(max_zr), positions, charges, bounds = (lower_bounds, upper_bounds), sigma = errors, absolute_sigma = False)
        pbar.update(+30)
        lower_popt, lower_pcov = curve_fit(wrapper_function(min_zr), positions, charges, bounds = (lower_bounds, upper_bounds), sigma = errors, absolute_sigma = False)
        pbar.update(+30)
        ...
        for i in range(len(positions)):
            lower_depth_scan_results[i] = charge_density_normalisation_with_reflection_and_smearing(positions[i], lower_popt[0], energy, max_zr)
            pbar.update(+(i/len(positions))*10)
            upper_depth_scan_results[i] = charge_density_normalisation_with_reflection_and_smearing(positions[i], upper_popt[0], energy, max_zr)
            pbar.update(+(i/len(positions))*10)
            abs_lower_depth_scan_results[i] = charge_density_normalisation_with_reflection_and_smearing(positions[i], min(beta_2_values), energy, max_zr)
            pbar.update(+(i/len(positions))*10)
            abs_upper_depth_scan_results[i] = charge_density_normalisation_with_reflection_and_smearing(positions[i], max(beta_2_values), energy, max_zr)
            pbar.update(+(i/len(positions))*10)
        pickle.dump([upper_beta, lower_beta, upper_beta_err, lower_beta_err, lower_depth_scan_results, upper_depth_scan_results, abs_lower_depth_scan_results, abs_upper_depth_scan_results], results_pickle)
        pbar.finish()
    else:
        results_pickle = open(bound_pickle, "rb")

        upper_beta, lower_beta, upper_beta_err, lower_beta_err, lower_depth_scan_results, upper_depth_scan_results, abs_lower_depth_scan_results, abs_upper_depth_scan_results = pickle.load(results_pickle)

    positions = positions.flatten()
    
    plt.clf()
    fig, ax = plt.subplots()
    ax.plot(positions*10**6, upper_depth_scan_results)
    ...
    plt.savefig("plots/depth_scan_numerical_aperture_confidence_levels.png")
    plt.clf()
    ...
    plt.savefig("plots/shaded_na.png")
    plt.clf()

def main():
    if __name__ == "__main__":
        #curve_fitting("x3y3")
        confidence_plot("x3y3", 2.4e-6, 3.4e-6)
