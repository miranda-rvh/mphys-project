...
def direct_charge_density_integrand(z, r, V, beta2, Ep, variable):
    l = WAVELENGTH
...
def reflected_charge_density_integrand(z, r, V, beta2, Ep, variable):
    l = WAVELENGTH
    NAv = EFFECTIVE_NUMERICAL_APERTURE
    tau = variable
    h = H
...
def interference_charge_density_integrand(z, r, V, beta2, Ep, variable):
    l = WAVELENGTH
    nr = REFRACTIVE_INDEX
    NAv = EFFECTIVE_NUMERICAL_APERTURE
    tau = variable
...
def array_charge_density_normalisation_with_reflection_and_smearing(V, beta, ep, variable):
    r_lim = R_LIMIT
    nr = REFRACTIVE_INDEX
    NAv = EFFECTIVE_NUMERICAL_APERTURE
    ...
    direct, error = integrate.dblquad(direct_charge_density_integrand, 0, r_lim, 0, z_lim, args=(V, beta, ep, variable))
    reflected, error = integrate.dblquad(reflected_charge_density_integrand, 0, r_lim, 0, z_lim, args=(V, beta, ep, variable))
    interference, error = integrate.dblquad(interference_charge_density_integrand, 0, r_lim, 0, z_lim, args=(V, beta, ep, variable))

    total_normalisation = direct + reflected + interference

    return np.array([total_normalisation])

def parallel_charge_density(V_values, beta, ep, variable):
    """Parallel computation of charge densities for multiple V values."""

    with mp.Pool(mp.cpu_count()) as pool:
        results = pool.starmap(array_charge_density_normalisation_with_reflection_and_smearing, [(V, beta, ep, variable) for V in V_values])
...
def fit_function(V, beta, ep,variable):
    result = parallel_charge_density(V, beta,ep, variable)

    return np.array(result)

def wrapper_function(test_variable):
    def tempfunc(V, beta, ep, variable = test_variable):
        return fit_function(V, beta, ep, test_variable)
    return tempfunc

def charge_density_normalisation_with_reflection_and_smearing(V, beta, ep, variable):
    r_lim = R_LIMIT
    nr = REFRACTIVE_INDEX
    NAv = EFFECTIVE_NUMERICAL_APERTURE
...
def data_file_read(detector_thickness, recalculate_exp_charges=False, plot = False, plot_sim = False, debug = False, mat_file = SIC_DEPTH_SCAN, filename = CHARGE_DATA_FILE_NAME, beta = BETA_2, tau = PULSE_DURATION, ep = PULSE_ENERGY):
    ...
    if plot:  
        ...
        if plot_sim:
            ...
            for i in range(len(experiment_positions)):
                normalisations[i] = charge_density_normalisation_with_reflection_and_smearing(experiment_positions[i], beta, ep, tau)
...
def curve_fitting(position, save_fig = "plots/depth_scan.png", range_pickle = "pickle_files/tau_range_fit.pickle"):
...
def confidence_plot(position, max_tau, min_tau, median_tau, range_pickle = "pickle_files/tau_range_fit.pickle", bound_pickle = "pickle_files/full_data_tau_pickle.pickle",recalculate = True):
    beta_data = np.genfromtxt(
            "beta_vals.txt", 
            delimiter=",", 
            dtype=[('coordinate', 'U10'), ('beta', 'f8'), ('error', 'f8')], 
    ...
    if recalculate:
        ...
        upper_popt, upper_pcov = curve_fit(wrapper_function(max_tau), positions, charges, bounds = (lower_bounds, upper_bounds), sigma = errors, absolute_sigma = False)
        lower_popt, lower_pcov = curve_fit(wrapper_function(min_tau), positions, charges, bounds = (lower_bounds, upper_bounds), sigma = errors, absolute_sigma = False)
        median_popt, median_pcov = curve_fit(wrapper_function(median_tau), positions, charges, bounds = (lower_bounds, upper_bounds), sigma = errors, absolute_sigma = False)
        ...
        for i in range(len(positions)):
            lower_depth_scan_results[i] = charge_density_normalisation_with_reflection_and_smearing(positions[i], lower_popt[0], energy, median_tau)
            upper_depth_scan_results[i] = charge_density_normalisation_with_reflection_and_smearing(positions[i], upper_popt[0], energy, median_tau)
            median_depth_scan_results[i] = charge_density_normalisation_with_reflection_and_smearing(positions[i], median_popt[0], energy, median_tau)

            abs_lower_depth_scan_results[i] = charge_density_normalisation_with_reflection_and_smearing(positions[i], min(beta_2_values), energy, median_tau)
            abs_upper_depth_scan_results[i] = charge_density_normalisation_with_reflection_and_smearing(positions[i], max(beta_2_values), energy, median_tau)
    ...
    plt.savefig("plots/depth_scan_tau_confidence_levels.png")
    plt.clf()
