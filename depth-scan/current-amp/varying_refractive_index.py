...
def data_file_read(detector_thickness, recalculate_exp_charges=False, plot = False, plot_sim = False, debug = False, mat_file = SIC_DEPTH_SCAN, filename = CHARGE_DATA_FILE_NAME, beta = BETA_2, nr = REFRACTIVE_INDEX, ep = PULSE_ENERGY):
    ...
    if plot:  
        ...
        if plot_sim:
            ...
            for i in range(len(experiment_positions)):
                normalisations[i] = charge_density_normalisation_with_reflection_and_smearing(experiment_positions[i], beta, ep, nr)
...
def curve_fitting(lower_bounds, upper_bounds, charge_file_name = CHARGE_DATA_FILE_NAME, mat_file = SIC_DEPTH_SCAN, save_fig = "plots/depth_scan.png", range_pickle = "refractive_index_range_fit.pickle"):
    if __name__ == "__main__":
        experiment_n_carriers, experimental_positions, errors = data_file_read(Z_LIMIT, filename = charge_file_name, mat_file = mat_file)
...
def confidence_plot(max_nr, min_nr, median_nr, energy, lower_bounds, upper_bounds, charge_data_file, mat_file, range_pickle = "refractive_index_range_fit.pickle", bound_pickle = "full_data_pickle.pickle",recalculate = True):

    range_pickle_file = open(range_pickle, "rb")
    nr_array, beta_2_values, beta_2_errors = pickle.load(range_pickle_file)

    charges, positions, errors = data_file_read(Z_LIMIT, filename = charge_data_file)
    ...
    if recalculate:
        ...
        upper_beta_err = np.sqrt(np.diag(upper_pcov))
        lower_beta_err = np.sqrt(np.diag(lower_pcov))
        median_beta_err = np.sqrt(np.diag(median_pcov))
        ...
        upper_beta, lower_beta, median_beta, upper_beta_err, lower_beta_err, median_beta_err, lower_depth_scan_results, upper_depth_scan_results, median_depth_scan_results, abs_lower_depth_scan_results, abs_upper_depth_scan_results = pickle.load(results_pickle)
        print("upper allowed: ", upper_beta, max_nr)
        print("lower allowed: ", lower_beta, min_nr)
        print("medium allowed: ", median_beta, median_nr)
        print("abs maximum allowed: ", max(beta_2_values), np.max(nr_array))
        print("abs minimum allowed: ", min(beta_2_values), np.min(nr_array))

    positions = positions.flatten()
    plt.clf()
    fig, ax = plt.subplots()
    ax.plot(positions, median_depth_scan_results)
    #ax.plot(positions, lower_depth_scan_results)
    #ax.plot(positions, upper_depth_scan_results)
    #ax.plot(positions, abs_lower_depth_scan_results)
    #sax.plot(positions, abs_upper_depth_scan_results)
    
    
    ax.fill_between(positions, abs_lower_depth_scan_results, abs_upper_depth_scan_results, color = 'grey', alpha = .2)
    ax.fill_between(positions, lower_depth_scan_results, median_depth_scan_results, color = 'b', alpha = .2)
    ...
    """full_nr_array = np.append(nr_array, [max_nr, min_nr, median_nr])
    full_beta_array = np.append(beta_2_values, [upper_beta, lower_beta, median_beta])
    full_beta_2_errs = np.append(beta_2_values, [upper_beta, lower_beta, median_beta])"""
    ...
    #ax.fill_between([2.615, 2.6266], [lower_beta-lower_beta_err, upper_beta-upper_beta_err], [lower_beta+lower_beta_err, upper_beta+upper_beta_err],alpha = 0.2, color = 'b')
...
def main():
    if __name__ == "__main__":
        #curve_fitting([1e-14, (1.023150e+03-4.433785e+00)*10**-12], [1e-11,(1.023150e+03+4.433785e+00)*10**-12],"ChargeFiles200V/200_x3y3.txt", "200V_Depth/TCT_x3y3.mat")
        confidence_plot(2.6155678343882305, 2.56100006558642, 2.6, 1.023150e+03, [1e-14, (1.023150e+03-4.433785e+00)*10**-12], [1e-11,(1.023150e+03+4.433785e+00)*10**-12],"ChargeFiles200V/200_x3y3.txt", "200V_Depth/TCT_x3y3.mat", recalculate = True)
